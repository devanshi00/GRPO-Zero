...
@torch.no_grad()
def rollout(
    model: AutoModelForCausalLM,  # Using Hugging Face model
    batch: MiniBatch,
    tokenizer: AutoTokenizer,  # Use AutoTokenizer instead of custom Tokenizer
    max_gen_len: int,
    ...
    # Prepare token tensor with padding
    ...
    for cur_pos in range(min_prompt_len, total_len):
        ...
        # Forward pass for logits (token generation)
        ...
        # Preserve prefix tokens
        next_token = torch.where(input_text_mask[:, cur_pos], tokens[:, cur_pos], next_token)
        next_token = torch.where(is_finished, pad_token_id, next_token)
        tokens[:, cur_pos] = next_token
        
        # Handle end token and finish flag
    ...
    for i in range(bsz // num_answer_per_question):
        for j in range(num_answer_per_question):
            ...
            # Use AutoTokenizer's detokenize function
